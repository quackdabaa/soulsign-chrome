	...
	>
		...
		<mu-sub-header>
			<mu-tooltip :content="'点击表单可展开更多内容，点击域名可跳转相应网址'" :placement="'top'">
				<mu-flex> 域名列表 </mu-flex>
			</mu-tooltip>
		</mu-sub-header>
		<mu-data-table
			selectable="true"
			select-all="true"
			:selects.sync="selects.chip"
			checkbox
			stripe
			:checkbox-col-width="20"
			:columns="columns"
			:data="task.result.detail"
		>
			...
			<mu-flex align-items="start" justify-content="start" fill wrap="wrap">
				<mu-tooltip :content="'配置复制内容'" :placement="'left'">
					<mu-flex align-items="center" justify-content="center">
						<mu-button slot="action" class="copy-config" flat color="primary" @click="copyConfig">
				...
				<mu-tooltip :content="'复制'" :placement="'right'">
					<mu-flex align-items="center" justify-content="center">
						<mu-button
							...
							flat
							color="primary"
							@click="copySelect(task.result.detail)"
						>
	...
	methods: {
		...
		copySelect(list) {
			...
			for (let idx = 0; idx < this.selects.chip.length; idx++) {
				...
				if (idx) message += ",";
				if (typeof item != "object") message += item;
				else {
					let item_picked = {};
		...
		copyConfig(config) {
			if (config === true || this.configCopy)
				this.selects.copy = ["url", "domain", "message", "errno"];
			if (config !== true) this.configCopy = !this.configCopy;
		},
		selectRemove(index) {
