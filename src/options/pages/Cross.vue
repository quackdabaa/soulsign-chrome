<template>
	<mu-container class="pages-cross" style="margin-bottom: 245px; max-width: 640px">
		...
		<div class="tar">
			...
			<mu-button color="primary" @click="add">添加</mu-button>
		</div>
		<mu-data-table :columns="columns" :data="cross_list" stripe>
			<template slot-scope="{row, $index}">
				...
				<td>
					<mu-switch
						:input-value="Boolean(row.cookie)"
						@change="saveCrose((row.cookie = row.cookie ? 0 : 2))"
					></mu-switch>
				</td>
				<td>
					<mu-switch
						:input-value="Boolean(row.enable)"
						@change="saveCrose((row.enable = row.enable ? 0 : 1))"
					></mu-switch>
				</td>
				<td>
					<mu-button small color="error" @click="del($index)">删除</mu-button>
				</td>
			</template>
		</mu-data-table>
	</mu-container>
	...
	methods: {
		add() {
			...
			for (let item of this.cross_list) {
				if (item.site == host) return this.$toast.error("站点已存在");
			}
